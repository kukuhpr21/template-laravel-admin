@props(['color', 'href'])
@php
    $baseClasses = 'inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent focus:outline-none disabled:opacity-50 disabled:pointer-events-none';
...
@endphp
<a href="{{ $href }}" {{ $attributes->merge(['class' => "{$baseClasses} {$colorClasses}"]) }}>{{ $slot }}</a>
